...
public class ProductDAO {
	...
	ArrayList<Product> productRetrieve(String productname) throws SQLException {
		...
		try {
			conn = connPool.getPoolConnection();
			stmt = conn
					.prepareStatement("SELECT * FROM shoppingproduct WHERE ProductName like '%" + productname + "%'");
			rset = stmt.executeQuery();
			while (rset.next()) {
				int ProductID = rset.getInt(1);
				String ProductType = rset.getString(2);
	...
	void productUpdate(int productid, String producttype, String productname, String explanation, int price,
			int inventory) {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rset = null;
